...
TEST_CASE(stdlib_uses_rx_files_as_canonical_source)
{
	...
	std::ostringstream core_catalog_text;
	std::ostringstream alloc_catalog_text;
	std::ostringstream std_catalog_text;
	glue_text << glue.rdbuf();
	core_catalog_text << core_catalog.rdbuf();
	alloc_catalog_text << alloc_catalog.rdbuf();
	std_catalog_text << std_catalog.rdbuf();
	REQUIRE(!contains(glue_text.str(), "const char *portable_stdlib_source()"));
	REQUIRE(!contains(core_catalog_text.str(), "FunctionDecl{"));
	REQUIRE(!contains(alloc_catalog_text.str(), "FunctionDecl{"));
	REQUIRE(!contains(std_catalog_text.str(), "FunctionDecl{"));
