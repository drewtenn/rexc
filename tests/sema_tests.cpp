...
TEST_CASE(sema_accepts_all_stdlib_alloc_helpers_as_bare_names)
{
	...
	auto result = analyze(
		...
		"  if can_allocate && !clone_empty && str_eq(joined, \"hello!\") && str_eq(owned_joined, \"hello!!\") && str_eq(number, \"-42\") && str_eq(truth, \"true\") && str_eq(letter, \"z\") { return remaining + used + capacity + clone_len + box_value + written + pushed + vec_len + vec_capacity + vec_value; }\n"
		"  return 0;\n"
		"}\n",
		diagnostics,
		options);
