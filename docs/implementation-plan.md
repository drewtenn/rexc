...
| [ ] | FE-105 | **NOT COMPLETE:** Model allocator as explicit value/parameter (interface contract) | FE-103 | **XL** *(was L; codex flagged: allocator-as-interface without traits is hard to scope)* | **BLOCKED:** low-level `Arena` / `arena_*` works, but `Vec::with_alloc(arena)` is missing and must be implemented/tested before this can be checked off | src/stdlib/alloc/alloc.rx, src/stdlib/stdlib.cpp, src/sema.cpp, src/lower_ir.cpp |

> **FE-105 verification note (2026-04-29):** partial only, not complete. The low-level explicit-arena foundation is present: `Arena` and `arena_*` are defined in `alloc.rx`, stdlib registration exposes `Arena`, and the FE-105-tagged tests for `arena_alloc` / `*Arena` type-checking pass. The written exit criterion is not satisfied: there is no `with_alloc` implementation or test outside docs/plans, and compiling a direct `Vec::with_alloc(&arena, 4)` probe fails with `unknown function 'Vec::with_alloc'`. Keep FE-105 open until the vector allocator pattern is implemented and covered by tests.
