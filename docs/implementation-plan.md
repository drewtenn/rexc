...
| [X] | FE-105 | Model allocator as explicit value/parameter (interface contract) | FE-103 | **XL** *(was L; codex flagged: allocator-as-interface without traits is hard to scope)* | allocator parameter type-checks; `Vec::with_alloc(arena)` pattern works; **scope held to a single concrete allocator type — no trait-shaped abstraction** | src/stdlib/alloc/alloc.rx, src/stdlib/stdlib.cpp, src/sema.cpp, src/lower_ir.cpp |
