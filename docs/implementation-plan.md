...
> **FE-103 status:** generic *function* monomorphization is done (per-instantiation mangled symbols, type inference from arg types, span-correct diagnostics — see `tests/ir_tests.cpp` and `tests/sema_tests.cpp`). Generic *struct* monomorphization (including recursive `Tree<T>` self-references) is **not yet implemented**; that's the remaining FE-103 work and is what `Vec<T>`/`HashMap<K,V>` (FE-109) will need.
