...
| [~] | FE-103 | Monomorphize generic fns and structs | FE-102 | XL | `id<i32>` and `id<bool>` link as separate symbols; recursive `Tree<T>` works; instantiation diagnostic carries span at the call site (not template body) | src/lower_ir.cpp, src/ir.cpp, src/types.cpp |
...
> **FE-103 status:** generic *function* monomorphization is done (per-instantiation mangled symbols, type inference from arg types, span-correct diagnostics). Non-recursive generic *struct* monomorphization is also done — `Box<i32>`/`Pair<i32, bool>`-shape templates instantiate at every type-position use, struct literals adopt the monomorph from the expected type, and per-instantiation layouts are computed independently in sema and lower_ir. **Recursive `Tree<T>` self-references** (struct that points to its own instantiation) is the remaining gap; it would require pre-registering the mangled name *before* substituting fields so `*Tree<T>` resolves to itself. That last piece is what unlocks `Vec<T>`/`HashMap<K,V>` (FE-109).
| [ ] | FE-105 | Model allocator as explicit value/parameter (interface contract) | FE-103 | **XL** *(was L; codex flagged: allocator-as-interface without traits is hard to scope)* | allocator parameter type-checks; `Vec::with_alloc(arena)` pattern works; **scope held to a single concrete allocator type — no trait-shaped abstraction** | src/types.cpp, src/sema.cpp, new src/stdlib/core/alloc.rx |
| [ ] | FE-106 | Refactor stdlib `alloc.rx` to expose arena allocator as a stdlib type (today: implicit static) | FE-105 | M | arena allocator round-trip smoke test passes | src/stdlib/alloc/alloc.rx |
| [ ] | FE-107 | Add `defer` statement and scope-cleanup lowering | FE-106 | M | defer runs on early return and on normal exit | grammar/Rexy.g4, src/parse.cpp, src/ast.cpp, src/sema.cpp, src/lower_ir.cpp |
| [ ] | FE-108 | Implement definite-assignment analysis | FE-107 | L | use-before-init emits error with span | src/sema.cpp |
