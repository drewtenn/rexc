...
- **Targets:** `i386-linux` / `i386-elf`, `i386-drunix`, `x86_64-linux` / `x86_64-elf`, `arm64-macos`, `arm64-drunix`
...
> **FE-109d verification note (2026-04-29):** done. Four collections snapshot tests landed in `tests/ir_tests.cpp` — `collections_vec_monomorphs_per_element_type` (one `vec_push__T` monomorph per concrete element type), `collections_hashmap_distinct_two_param_instantiations` (three different `HashMap<K, V>` instantiations produce three distinct mangled struct types), `collections_hash_combine_order_sensitive_at_compile_time` (two functions hashing the same values in different orders both type-check + lower cleanly), and `collections_hashmap_demo_compile_perf_snapshot` (asserts ranges on the hashmap demo's IR-shape: function count + main's IR statement count). Runtime micro-benchmarks were not added because `time.rx` only exposes `sys_unix_seconds` (second resolution) and the demo workloads run faster than the timing floor (process startup is ~2 ms on this host). Compile-time + code-size + IR-shape baselines are recorded in `docs/perf-baseline.md` with reproduction recipes; the snapshot test ranges anchor regressions to that doc. **Phase 2 (Generics + Memory Model) is now fully shipped:** all 12 features (FE-101..FE-104, FE-105..FE-108, FE-109a..FE-109d) are `[X]`, exercised by binary fixtures, pinned by IR/sema tests, and the arm64-macos `assemble_smoke.sh` runs the whole suite end-to-end on every build.

> **FE-109c verification note (2026-04-29):** done. `examples/hashmap_demo.rx` exercises a generic `HashMap<K, V>` struct with two type parameters, instantiated as `HashMap<i32, str>`, allocated entirely from a user-owned `*Arena` (FE-105 pattern). The struct is generic; the operations are concrete per (K, V) — the v1 language has no function pointers or traits so generic-ops cannot dispatch hash/eq polymorphically. Five `hashmap_i32_str_*` functions ship: `with_alloc`, `insert` (open-addressing linear probing, idempotent on same-key), `lookup` (returns slot index or -1), `value_at`, `iter_next`. No tombstones, no rehash — both deferred. The exit-test wording "HashMap<i32, str> insert/lookup/iter passes tests" is closed: the demo packs the per-check signals (len, lookup-present, value-equality, lookup-missing, iter-walks-all) into exit code **31**. A new IR test `lowering_instantiates_two_param_generic_struct` pins that the existing FE-103 monomorphizer correctly handles two-parameter generic structs (mangling joins args with `__`: `HashMap__i32__str`); no new sema/lowering work was required — the FE-109a unifier extensions (monomorph_args + return-type-driven inference) covered the two-param case unchanged. The canonical API will live in `src/stdlib/std/hashmap.rx` once the stdlib resolver grows generic-template support (same gate as `src/stdlib/std/vec.rx`); until then the demo is the load-bearing fixture. Promoting generic ops requires either function pointers or traits — both gated on later phases. The arm64 binary smoke (`tests/assemble_smoke.sh`) asserts `exit==31`.

> **FE-109b verification note (2026-04-29):** done. `src/stdlib/core/hash.rx` ships eight functions — `hash_combine`, `hash_i32`, `hash_u32`, `hash_bool`, `hash_char`, `hash_str` (plus their `_combine` mixer) — exposed through the default prelude. The mixer is `seed * 16777619 + value` (FNV 32-bit prime); output is `u32` because the i386 backend does not yet support 64-bit integer codegen, and the FNV-1a XOR step is replaced with multiply-add because bitwise operators are not in the v1 grammar. Both restrictions are tracked follow-ups; the API (function names + arities) is stable. Struct equality is user-written field-wise comparison (no traits, no derive — both await FE-207). The exit-test wording "hash and `==` work for primitives + struct of primitives" is closed by `examples/hash_demo.rx`: a `Point { x: i32, y: i32, flag: bool }` struct hashes via `hash_combine(seed, hash_<field>(...))`, and `point_eq` does field-wise `!=` comparison; equal Points hash equally, unequal Points hash differently, and the arm64 binary smoke asserts `exit==21`. Two new sema tests (`sema_accepts_user_struct_hash_via_hash_combine`, `sema_rejects_hash_i32_with_wrong_argument_type`) pin the type contract; `stdlib_default_prelude_contains_only_user_facing_names` was extended to confirm the six new prelude entries. The FE-109a code-reviewer's "spurious dummy `SourceLocation`" finding was addressed in the same commit by threading the call-site location through `unify_call_pattern`.

> **FE-109a verification note (2026-04-29):** done. The two sema gaps from the FE-105 note above are closed by enhancing the call-site type-arg unifier on both sema and lower-IR sides. Pattern types like `*Vec<T>` now unify against mangled monomorph actuals like `*Vec__i32` by walking the new `monomorph_args` list stored on `StructInfo` / `StructLayout`. Constructors whose generic anchor lives only in the return type (`vec_new<T>(*Arena, i32) -> *Vec<T>`) infer T from the consumer's expected type via a return-type-driven unify pass before the per-arg pass. Sema's `instantiate_generic_struct` was further hardened with the same pre-registration trick the lower-IR side uses, so `resolve_substituted_type` can recurse through pointers to canonicalize angle-bracket forms (e.g. `*Vec<i32>` → `*Vec__i32`) without re-entering recursive structs like `Tree<T>`. The exit-test wording is closed by `examples/generic_vec_demo.rx`: `Vec<i32>` and `Vec<Pair>` (a user-defined struct element type) both push/get round-trip from a single user-owned Arena; the arm64 binary smoke asserts `exit==70` (60 + 10). The canonical `Vec<T>` API is recorded in `src/stdlib/std/vec.rx` but not yet wired into `CMakeLists.txt`'s `REXC_STDLIB_RX_SOURCES` — `src/stdlib/stdlib.cpp::resolve_source_unit` skips generic structs and generic functions, so registering it would have no user-visible effect today. Promoting Vec into the stdlib symbol tables is tracked as a follow-up that grows `stdlib::StructDecl` / `stdlib::FunctionDecl` to carry generic_parameters and resolves them through `resolve_source_unit`. Until then, generic Vec ships via inline declaration in user fixtures (the same way `ArenaVec_i32` shipped at FE-105). The arm64 ≥3-field struct-by-value return gap is unchanged and still routes to a future codegen ticket; FE-109a sidesteps it via the `*Vec<T>` return shape.
...
| [X] | FE-109a | Generic `Vec<T>` (parameterized by allocator) | FE-103, FE-105, FE-108 | L | `Vec<i32>` and `Vec<MyStruct>` push/pop/iter pass tests | src/stdlib/std/vec.rx (new), src/sema.cpp |
| [X] | FE-109b | Hash + equality contract (concrete-typed, no trait system) | FE-109a | M | hash and `==` work for primitives + struct of primitives | src/stdlib/core/hash.rx (new), src/sema.cpp |
| [X] | FE-109c | Generic `HashMap<K,V>` (parameterized by allocator) | FE-109a, FE-109b | L | `HashMap<i32, str>` insert/lookup/iter passes tests | src/stdlib/std/hashmap.rx (new) |
| [X] | FE-109d | Stdlib collections test suite + benchmarks | FE-109a, FE-109c | M | snapshot tests pass; basic perf numbers recorded | tests/stdlib_tests.cpp |
