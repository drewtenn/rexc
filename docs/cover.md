...
Rexy is the language side of the Drunix experiment, and `rexc` is its compiler.
Drunix asks what it takes to build an operating system from the first boot
instruction upward. Rexy asks the companion question: once the operating system
can run user programs, what does it take to build our own language that can
