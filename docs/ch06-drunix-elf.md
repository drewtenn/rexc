...
Rexc had text that described instructions. After assembly, the build has an
ELF object file containing real i386 machine code.

### Why the Runtime Is Part of the Program

A Rexc function named `main` is not the first instruction Drunix runs in a user
...
That startup path is why Rexc output must be linked with the Drunix runtime.
...
"Rexc program" loader. Once the final link succeeds, the kernel sees an ELF
